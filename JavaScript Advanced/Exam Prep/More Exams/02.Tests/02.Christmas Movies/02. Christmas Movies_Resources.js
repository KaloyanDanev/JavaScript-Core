...
describe("Christmas movies", function() {
    let sampleInstance;
    beforeEach(function () {
        sampleInstance = new ChristmasMovies('Pesho');
    });
    it('testing constructor movieCollection', function () {
        expect(sampleInstance.movieCollection).to.eql([]);
    });
    it('testing constuctor actors', function () {
        expect(sampleInstance.actors).to.eql([]);
    });
    it('test constructor watched', function () {
        expect(typeof sampleInstance.watched).to.equal('object');
    });
    it('should buy movie', function () {
        const result = sampleInstance.buyMovie('film',['Macaulay','Joe','Daniel']);
        expect(result).to.deep.equal(`You just got film to your collection in which Macaulay, Joe, Daniel are taking part!`)
    });
    it('should throw error buyMovie', function () {
        const result = sampleInstance.buyMovie('film',['Macaulay','Joe','Daniel']);
        expect(() => result.to.throw(`You already own film in your collection!`));
    });
    it('should test mostStarredActor', function () {
        expect(() => sampleInstance.mostStarredActor.to.throw(`You already own film in your collection!`));
    });
    it('should test watch movie error', function () {
        expect(() => sampleInstance.watchMovie.to.throw(`No such movie in your collection!`))
    });
    it('should discardmovie', function () {
        const result = sampleInstance.buyMovie('film',['Macaulay','Joe','Daniel']);
       let movie = sampleInstance.movieCollection.length;
       expect(movie).to.equal(1);
    });
    it('should starred error ', function () {
        expect(() => sampleInstance).to.throw(`You have not watched a movie yet this year!`)
    });

    it('should exist', function () {
        expect(sampleInstance.discardMovie).to.exist;
        expect(sampleInstance.discardMovie).to.be.a("function");

    });
    it('should be a fun moststar ', function () {
        expect(sampleInstance.mostStarredActor).to.exist;
        expect(sampleInstance.mostStarredActor).to.be.a("function");
    });
    it('should be a fun ', function () {
